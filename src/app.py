...
st.title("🏎️ F1 Telemetry Dashboard (Technical Edition)")
...
def interactive_speed_plot(tel1, tel2, drv1, drv2):
    df1 = tel1[['Distance','Speed']].copy()
    df1['Driver'] = drv1
    df2 = tel2[['Distance','Speed']].copy()
    df2['Driver'] = drv2
    df = pd.concat([df1, df2])
    fig = px.line(df, x='Distance', y='Speed', color='Driver', title="Lap Speed Comparison")
    st.plotly_chart(fig)

def interactive_gforce_plot(tel1, tel2, drv1, drv2):
    tel1['ax'] = tel1['Speed'].diff() / tel1['Time'].diff().dt.total_seconds()
    tel2['ax'] = tel2['Speed'].diff() / tel2['Time'].diff().dt.total_seconds()
    df1 = tel1[['Distance','ax']].copy()
    df1['Driver'] = drv1
    df2 = tel2[['Distance','ax']].copy()
    df2['Driver'] = drv2
    df = pd.concat([df1, df2])
    fig = px.line(df, x='Distance', y='ax', color='Driver', title="Longitudinal Acceleration")
    st.plotly_chart(fig)
...
def track_map_heatmap(lap_obj, driver, metric='Speed'):
    ...
    if 'X' not in telemetry.columns or 'Y' not in telemetry.columns:
        st.warning(f"No track coordinates for {driver}")
        return
...
if st.button("Compare Laps"):
    ...
    if tel1 is None or tel2 is None:
        ...
        tab1, tab2, tab3, tab4 = st.tabs(["Speed", "G-Force", "Track Map", "Sector Delta"])
        with tab1:
            interactive_speed_plot(tel1, tel2, driver1, driver2)
        with tab2:
            interactive_gforce_plot(tel1, tel2, driver1, driver2)
        with tab3:
